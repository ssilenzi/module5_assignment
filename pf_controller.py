...
def distance_to_closest_border(x0, y0, theta, map_limits):
    x_min, x_max, y_min, y_max = map_limits
    m = np.tan(theta)
    c = y0 - np.tan(theta) * x0
    if theta <= np.pi/2:
        y1 = m*x_max + c
        if y1 < y_max:
            return distance(x0,y0,x_max,y1)
        else:
            x1 = (y_max - c )/ m
            return distance(x0,y0,x1,y_max)
    elif theta <= np.pi:
        y1 = m*x_min + c
        if y1 < y_max:
            return distance(x0,y0,x_min,y1)
        else:
            x1 = (y_max - c )/ m
            return distance(x0,y0,x1,y_max)
        ...
        if y1 > y_min:
            ...
            x1 = (y_min - c )/ m
            return distance(x0,y0,x1,y_min)
    else:
        y1 = m*x_max + c
        if y1 > y_min:
            return distance(x0,y0,x_max,y1)
